C     This code comes from "Prediction of Tropospheric Radio
C     Transmission Loss Over Irregular Terrain, A Computer Method-1968",
C     Longley and Rice.  According to the report, the original program
C     was written in Fortran IV on a Control Data Corporation CDC-3600.
C
C     This is the original code without errata from the report applied.
C     It is been updated enough to compile with an f77 compiler.
C     The only interest in code with known bugs is that the Longley-Rice
C     report has test output generated from that code.  This program can
C     be run for comparison.  Differences can be seen, almost certainly
C     due to differences in representation of real numbers on the 1968
C     Control Data Corporation CDC-3600 and present day machines.
C
C     On a linux machine, compile with:
C        f77 -o lr68bugs lr1968-f77-bugs.f
C     Run with: lr68bugs
C
...
59      FORMAT (2X,'OHIO   NS=312.'//)
        IF (IX .EQ. 1) WRITE ( 6,57)
        IF (IX .EQ. 2) WRITE ( 6,58)
        IF (IX .EQ. 3) WRITE ( 6,59)
        DO 400 I=1,9
          KK=0
          DO 300 IZ=1,6
            ...
            H1G=4.
            H1E=4.
            IF (IX .NE. 3 .AND. I .EQ. 9) THEN
              H1G=3.3
              H1E=3.3
            ENDIF
            IF (IX .EQ. 3 .AND. I .GT. 6) THEN
              H1G=4.24
              H1E=4.24
            ENDIF
            IF (IX .EQ. 3 .AND. I .EQ. 9) THEN
              H1G=3.68
              H1E=3.68
            ENDIF
            H2G=3.
            ...
            IF (I .EQ. 3 .OR. I .EQ. 6) THEN
              ...
              H2E=9.
            ENDIF
            IF (IX .EQ. 3 .AND. I .EQ. 7) THEN
              H2G=1.
              H2E=1.
            ENDIF
            IF (IX .NE. 3 .AND. I .EQ. 7) THEN
              H2G=.55
              H2E=.55
            ENDIF
            IF (IX .NE. 3 .AND. I .EQ. 8) THEN
              H2G=1.7
              H2E=1.7
            ENDIF
            IF (IX .NE. 3 .AND. I .EQ. 9) THEN
            ...
            DLS2=SQRT(.002*A*H2E)
            DLS=DLS1+DLS2
            DL1=DLS1*EXP(-.07*SQRT(DH/H1E))
            DL2=DLS2*EXP(-.07*SQRT(DH/H2E))
            DL=DL1+DL2
...
C           WRITE OUTPUT
C
60          FORMAT (4X,'F='F6.1,'   DH='F6.2,'   H1G='F6.2,
     C      '   H2G='F6.2,'   TE='F10.6,'   DX='F8.2)
            WRITE ( 6,60) F,DH,H1G,H2G,TE,DX
61          FORMAT (4X,'AE='F8.2,'   K1='F10.5,'   K2='F10.5,
     C      '   DLS='F8.2,'   ALS='F8.2)
            WRITE ( 6,61) AE,K1,K2,DLS,ALS
            ADX=AED+MD*DX
62          FORMAT (4X,'AED='F8.2,'   MD='F10.5,'   AES='F8.2,
     C      '   MS='F10.5,'   ADX='F8.2)
            WRITE( 6,62) AED,MD,AES,MS,ADX
63          FORMAT (4X,'D'6F10.2)
64          FORMAT (4X,'A'6F10.2)
            WRITE ( 6,63) (SD(JZ), JZ=1,KK)
            WRITE ( 6,64) (SA(JZ), JZ=1,KK)
            WRITE ( 6,56)
300       CONTINUE
400     CONTINUE
500   CONTINUE
...
19    IF(X1 .GT.0. .AND.X1 .LE.200. .AND.K1 .GT. .00001 .AND. K1 .LT. 1.
     C) THEN
      ...
      ENDIF
22    IF(X2 .GT.0. .AND.X2 .LE.200 .AND.K2 .GT. .00001 .AND. K1 .LT. 1.
     C) THEN
...
20    DH=0.
      ...
      DL2=DLS2*EXP(-.07*SQRT(DH/H2E))
      DL=DL1+DL2
      TE1=(.00065/DLS1)*((DLS1/DL1-1.)*DH-3.077*H1G)
      TE2=(.00065/DLS2)*((DLS2/DL2-1.)*DH-3.077*H2G)
      TE=MAX((TE1+TE2),(-DL/A))
      T=TE+D/A
      AH5S=AH5
      MSS=MS
...
C
      ...
      SA0=A0
      SA1=A1
      W=(AMIN1(H1E,H2E)/AMAX1(H1E,H2E))/(1.+F*DH*.0001)
      W=1./(1.+F*DH*.0001)
