...
C     given in the 1968 errata are applied.  The original, uncorrected
C     lines remain, commented out with "C!!!".  Changes were made,
C     as minimal as possible, for compilation under Fortran 77, a
C     compiler more readily available.  Now you can see Mrs. Longley's
C     code running again for the first time since 1968!
C
C     On a linux machine, compile with: f77 -o lr1968 lr1968-f77.f
C     Run with: lr1968
C
...
C!!!  DL2=DLS2*EXP(-.07*SQRT(DH/H2E))
      DL1=DLS1*EXP(-.07*SQRT(DH/MAX(5.,H1E)))
      DL2=DLS2*EXP(-.07*SQRT(DH/MAX(5.,H2E)))
      DL=DL1+DL2
